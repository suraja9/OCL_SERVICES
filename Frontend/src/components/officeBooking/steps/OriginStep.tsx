...
interface OriginStepProps {
  ...
  originAreas: string[];
  originMobileDigits: string[];
  onOriginDigitChange: (index: number, value: string) => void;
...
const OriginStep: React.FC<OriginStepProps> = ({
  ...
  const handleOriginDigitKeyDown = (index: number, e: React.KeyboardEvent) => {
    if (e.key === 'Backspace') {
      if (!originMobileDigits[index] && index > 0) {
        ...
        newDigits[index - 1] = '';
        onOriginDigitChange(index - 1, '');
      } else if (originMobileDigits[index]) {
        const newDigits = [...originMobileDigits];
        newDigits[index] = '';
        onOriginDigitChange(index, '');
      }
  ...
  return (
    <div className={cn('space-y-6', isDarkMode ? 'text-slate-100' : '')}>
                ...
                <div className="flex items-center gap-0.5 sm:gap-1 min-w-max">
                  {originMobileDigits.map((digit, index) => (
                    <input
                      ...
                      maxLength={1}
                      value={digit}
                      onChange={(e) => onOriginDigitChange(index, e.target.value)}
                      onKeyDown={(e) => handleOriginDigitKeyDown(index, e)}
                      onPaste={index === 0 ? handleOriginDigitPaste : undefined}
              ...
              <div className="flex items-center justify-center gap-1.5 sm:gap-2 md:gap-3 mb-3 sm:mb-4">
                {originOtpDigits.map((digit, index) => (
                  <input
                    ...
                    onFocus={(e) => e.target.select()}
                    className={cn(
                      "h-9 w-9 sm:h-10 sm:w-10 md:h-12 md:w-12 rounded-lg border-2 text-center text-sm sm:text-base md:text-lg font-bold transition-all duration-200",
            ...
            <>
              ...
              <div className="px-5 py-5 space-y-3">
                <div className="grid gap-3 md:grid-cols-2">
                  ...
                  <FloatingInput
                    label="Mobile No. :"
                    value={`+91 ${originMobileDigits.join('')}`}
                    onChange={() => {}}
                    disabled
                  ...
                  <FloatingInput
                    ...
                    isDarkMode={isDarkMode}
                  />
                  {originPincode.length === 6 && originAreas.length > 0 ? (
                    <FloatingSelect
                      label="Area"
                      value={data.area}
      ...
      {/* Next Button - Only show when form is complete */}
